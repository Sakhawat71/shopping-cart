...
function porductPriceCalculet(){
    ...
    const totalPrice = gteInputValue('phone') * 1200 + gteInputValue('case') * 60;

    document.getElementById('total-price').innerText = '$'+totalPrice;
    const taxcount = totalPrice * 0.1;
    const tax = Math.round(taxcount);
...
function gteInputValue(porduct){
    const porductInput = document.getElementById(porduct + '-count');
    const porductCount = parseInt(porductInput.value);
    return porductCount;
};
